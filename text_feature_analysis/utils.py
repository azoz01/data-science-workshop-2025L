...
def get_available_cols(
    text_feature_cols: List[str], model_name: str, male_df: pd.DataFrame, female_df: pd.DataFrame
):
    available_cols = [
        col for col in text_feature_cols if col in male_df.columns and col in female_df.columns
    ]
    missing_cols = set(text_feature_cols) - set(available_cols)
...
def compute_wilcoxon_pvalues(
    ...
    wilcoxon_pvalues = {
        feature: pvalue
        for feature, pvalue in sorted(wilcoxon_pvalues.items(), key=lambda item: item[1])
    }
...
def sort_dict_by_abs_value(unsorted_dict: dict) -> dict:
    sorted_dict = dict(
        sorted(
            {
                feature: value for feature, value in unsorted_dict.items() if not pd.isna(value)
            }.items(),
...
def get_sorted_cohens_d(
    male_df: pd.DataFrame, female_df: pd.DataFrame, text_feature_cols: dict
) -> dict:
    cohens_ds = compute_cohens_d(male_df, female_df, text_feature_cols)
